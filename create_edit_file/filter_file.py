...
class File_Filter:
	def __init__(self,file):
		...
		if self.file_type in VIDEO_TYPES:
			self.video_saver = cv.VideoWriter(self.write_file_path + 'output' + "." +  'mp4', 
								 cv.VideoWriter_fourcc(*'h264'),
								 50, file.get('dimensions')[-2::-1])
		self.is_image = 0
	...
	def file_render(self, color):
		try:
			...
			while True:
				...
				if not status and not self.is_image:
	...
	def get_reader_obj(self):
		if self.file_type in IMAGE_TYPES:
			obj =  cv.imread(self.file_path + 'file' +'.'+  self.file_type)
			self.is_image = 1
		elif self.file_type in VIDEO_TYPES:
			obj =  cv.VideoCapture(self.file_path + 'file' +'.'+ self.file_type)
			self.is_image = 0
		return obj
